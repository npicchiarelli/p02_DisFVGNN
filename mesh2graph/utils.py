...
def add_boundary_points_old(graph_vc: Data, directory: str, verbose: bool = False) -> Data:
    ...
    pos_dict = {}
    for i, cell in enumerate(mesh.cell_faces):
        if mesh.is_cell_on_boundary(i, b'bottom'):
            u.append(i)
            for face in mesh.cell_faces[i]:
                if mesh.is_face_on_boundary(face, b'bottom'):
                    print(f"Cell {i} is on the bottom boundary and has face {face} on the bottom boundary")
                    boundary_index += 1
                    pos_dict[boundary_index] = np.mean(mesh.points[mesh.faces[face]], axis=0)
                    v.append(boundary_index)
        if mesh.is_cell_on_boundary(i, b'left'):
            u.append(i)
            for face in mesh.cell_faces[i]:
                if mesh.is_face_on_boundary(face, b'left'):
                    print(f"Cell {i} is on the left boundary and has face {face} on the left boundary")
                    boundary_index += 1
                    pos_dict[boundary_index] = np.mean(mesh.points[mesh.faces[face]], axis=0)
                    v.append(boundary_index)
        if mesh.is_cell_on_boundary(i, b'top'):
            u.append(i)
            for face in mesh.cell_faces[i]:
                if mesh.is_face_on_boundary(face, b'top'):
                    print(f"Cell {i} is on the top boundary and has face {face} on the top boundary")
                    boundary_index += 1
                    pos_dict[boundary_index] = np.mean(mesh.points[mesh.faces[face]], axis=0)
                    v.append(boundary_index)
        if mesh.is_cell_on_boundary(i, b'right'):
            u.append(i)
            for face in mesh.cell_faces[i]:
                if mesh.is_face_on_boundary(face, b'right'):
                    print(f"Cell {i} is on the right boundary and has face {face} on the right boundary")
                    boundary_index += 1
                    pos_dict[boundary_index] = np.mean(mesh.points[mesh.faces[face]], axis=0)
                    v.append(boundary_index)
    ...
    pos_b = torch.cat([graph_vc.pos, torch.tensor([pos_dict[i] for i in range(mesh.num_cell, boundary_index+1)])], dim=0)
